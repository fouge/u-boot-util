...
public class SerialComPort {
	 ...
	 * than one open connection (using more than one instance of {@link SerialComPort}
	 ...
	 */
	public SerialComPort(int id, int divider) {
	...
	public boolean connect(String portName, int speed) {
		...
		try {
			...
			if (portIdentifier.isCurrentlyOwned()) {
				...
				reader.start();
				connected = true;
				System.out.println("connection on " + portName
						+ " established");
				conn = true;
			}
	 ...
	 * @return <b>true</b> if the message could be sent, <b>false</b> otherwise
	 *         or if one of the numbers is equal to the #{@link SerialComPort#divider}
	 *         .
	 */
	public boolean writeSerial(int numBytes, int message[]) {
		...
		if (success && isConnected()) {
			...
			System.out.println("The message contains the divider.");
		} else {
			System.out.println("No port is connected.");
		}
		return success;
